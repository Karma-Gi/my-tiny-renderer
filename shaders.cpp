...
std::pair<bool, TGAColor> PhongShader::fragment(const vec3 bar, const int x, const int y) const
{
    // 插值观察空间位置
    const vec3 position =
    ...
    // SSAO only attenuates the indirect ambient term.
    const int pixel_index = x + y * framebuffer_width;
    const double ao = std::clamp(ao_buffer[pixel_index], 0., 1.);
...
NormalShader::NormalShader(const Model &m)
    : model(m)
{
...
std::pair<bool, TGAColor> NormalShader::fragment(const vec3 bar, int, int) const
{
    const vec3 normal = normalized(varying_normal[0] * bar.x + varying_normal[1] * bar.y + varying_normal[2] * bar.z);

    TGAColor color{};
    color[0] = static_cast<std::uint8_t>(std::clamp(normal.x * 0.5 + 0.5, 0., 1.) * 255.);
    color[1] = static_cast<std::uint8_t>(std::clamp(normal.y * 0.5 + 0.5, 0., 1.) * 255.);
